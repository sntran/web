defmodule Web.ReadableStreamTest do
  ...
  describe "ReadableStream" do
    ...
    test "Enumerable suspend", %{stream: stream, controller_pid: pid} do
      ...
      {:suspended, "a", next} = Enumerable.reduce(stream, {:cont, ""}, fn 
        x, "" -> {:suspend, x}
        x, acc -> {:cont, acc <> x}
      end)
  ...
  describe "Controller" do
    ...
    test "edge cases" do
      ...
      {:ok, p2} = Controller.start_link()
      Controller.enqueue(p2, "a")
      Controller.close(p2) # This sends flush_requests, but queue not empty
      ...
      Controller.read(p4) # queue was 2, now 1. 1 < 1 is false.
  ...
  describe "Reader" do
    test "errors", %{stream: stream, controller_pid: pid} do
       ...
       assert_raise TypeError, "The stream is errored.", fn -> ReadableStreamDefaultReader.read(reader) end
       ...
       assert_raise TypeError, "The stream is errored.", fn -> ReadableStreamDefaultReader.read(r2) end
       ...
       assert_raise TypeError, ~r/Unknown error/, fn -> ReadableStreamDefaultReader.release_lock(r4) end
    end
    ...
    test "PID mismatch", %{controller_pid: pid} do
       ...
       assert_raise TypeError, ~r/longer locked/, fn -> ReadableStreamDefaultReader.release_lock(r) end
    end
